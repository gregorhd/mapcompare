...
"""Plot figure using Bokeh's figure.patches method and the WebGL backend, which currently does not seem to support the Patches glyph.
...
def renderFigure(buildings_in, buildings_out, rivers, basemap=basemap, savefig=savefig, db_name=db_name, viz_type=viz_type):
    ...
    if savefig:
        ...
        output_file(filename=outputdir + viz_type + "bokeh_webgl" + " (" + db_name + ").html")
    else:
        pass
