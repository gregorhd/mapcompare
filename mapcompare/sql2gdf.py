...
def sql2gdf(db_name, password):
    ...
    sql1 = "WITH buffer as (SELECT ST_Union(ST_Buffer(f.wkb_geometry, 500)) as geom FROM public.ax_fliessgewaesser as f) SELECT g.use, g.wkb_geometry as geom FROM public.ax_gebaeude as g JOIN buffer ON ST_Within(g.wkb_geometry, buffer.geom) OR ST_Intersects(g.wkb_geometry, buffer.geom);"
